...
@[simp] theorem safe_baseFine : SafeConfig baseFine := by
  simpa [SafeConfig, baseFine, capSet, obsFun] using fine_safe_base

@[simp] theorem safe_expandedFine : SafeConfig expandedFine := by
  exact fiberSafe_of_capability_expansion Good Set.univ (by simp [baseCap, expandedCap]) fine_safe_base

@[simp] theorem safe_expandedCoarse : SafeConfig expandedCoarse := by
  simpa [SafeConfig, expandedCoarse, capSet, obsFun] using coarse_safe_expanded

@[simp] theorem unsafe_baseCoarse : ¬ SafeConfig baseCoarse := by
  simpa [SafeConfig, baseCoarse, capSet, obsFun] using coarse_unsafe_base
...
theorem forgetting_admissible_when_expanded :
  ...
  intro q hq hx
  simpa [Win, forget, expandedFine] using safe_expandedCoarse
...
theorem forgetting_inadmissible_when_base :
  ...
  intro h
  have htarget : Win (forget baseFine) () := h () (by simp [Contract]) (by simpa [Win] using safe_baseFine)
  exact unsafe_baseCoarse (by simpa [Win, forget, baseFine] using htarget)
...
theorem capability_loss_after_forgetting_is_inadmissible :
  ...
  have htarget : Win (loseCapability expandedCoarse) () :=
    h () (by simp [Contract]) (by simpa [Win] using safe_expandedCoarse)
  exact unsafe_baseCoarse (by simpa [Win, loseCapability, expandedCoarse] using htarget)
