...
This file formalizes the role-level quotient of a finite deterministic staged family:
...
IMPORTANT SCOPE: this V1 kernel proves the combinatorics of the staged role quotient.  It does
not by itself prove that the quotient is induced by a nontrivial world/action/observation system;
that realization is the next kernel target.
...
T5f  Alternation depth is unbounded across the staged quotient family.
-/
...
/-- T5c — there is no other successful role ordering in this staged quotient family. -/
...
/-- Simultaneous induction package: the canonical trace alternates, and adding a leading REPAIR
also preserves alternation because a nonempty canonical trace begins with PROBE. -/
theorem canonical_alternating_pair (k : Nat) :
    Alternating (canonical k) ∧ Alternating (repair :: canonical k) := by
  induction k with
  | zero => simp [canonical, Alternating]
  | succ k ih =>
      have hcan : Alternating (canonical (k + 1)) := by
        simp [canonical, Alternating, ih.2]
      refine ⟨hcan, ?_⟩
      change repair ≠ probe ∧ Alternating (canonical (k + 1))
      exact ⟨by decide, hcan⟩

/-- The canonical trace alternates at every adjacent position. -/
theorem canonical_alternating (k : Nat) : Alternating (canonical k) :=
  (canonical_alternating_pair k).1
